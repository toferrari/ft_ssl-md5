...
/*   Updated: 2019/04/16 15:45:34 by tferrari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_hash	init_variable_h_sha224(t_hash hash)
{
	hash.h_init[0] = 0xc1059ed8;
	hash.h_init[1] = 0x367cd507;
	hash.h_init[2] = 0x3070dd17;
	hash.h_init[3] = 0xf70e5939;
	hash.h_init[4] = 0xffc00b31;
	hash.h_init[5] = 0x68581511;
	hash.h_init[6] = 0x64f98fa7;
	hash.h_init[7] = 0xbefa4fa4;
	return (hash);
}

t_hash	compute_sha224(t_hash hash, size_t i, uint32_t *w)
{
	uint32_t s1;
	uint32_t ch;
	uint32_t tmp;
...
t_hash	calculate_h_sha224(t_hash hash, uint8_t *m)
{
	int			i;
	size_t		j;
	uint32_t	w[64];
	...
	i = -1;
	while (++i < 64)
		hash = compute_sha224(hash, i, w);
	hash = update_h(hash, 8);
	return (hash);
}

t_hash	create_padding_sha224(t_hash hash)
{
	size_t		mod_h;
	uint64_t	size_hash;
...
t_hash	sha224(t_hash hash)
{
	...
	hash = create_padding_sha224(hash);
	hash = init_variable_h_sha224(hash);
	...
	while (0 < hash.len_octet_h)
	{
		...
		hash = calculate_h_sha224(hash, hashing);
		hash.len_octet_h -= 64;
		hashing = hashing + 64;
	}
